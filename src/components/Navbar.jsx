...
const Navbar = () => {
  ...
  return (
    <header className='flex justify-between z-50 p-2 w-full sm:items-center md:px-14 lg:px-24 fixed top-0 bg-white'>
      <img src={logo} alt="logo"  className='h-6 mr-10 sm:h-8'/>
        <nav className={` ${menuState} absolute z-50 sm:w-full sm:justify-between sm:flex sm:flex-row sm:p-0 sm:bg-white sm:top-0 sm:left-0 sm:translate-x-0 top-10 left-1/2 -translate-x-1/2 sm:relative bg-blue-950 text-white font-semibold w-[calc(100%-16px)] text-center py-10 rounded-lg`}>
