...
const Statistics = () => {
  ...
  return (
    <div className=' bg-gray-200 px-2 md:px-14 lg:px-24 z-10'>
        <Form />   
        ...
        <div className='flex flex-col gap-16 py-10 relative md:flex-row md:gap-6 lg:gap-10 md:pb-32 '>
            <div className='bg-cyan-400 w-2 h-4/5 left-1/2 absolute mx-auto flex md:w-4/5 md:h-2 md:left-[10%] md:top-1/3'></div>
            {
              statistics.map(item =>
                <div key={item.id}  className={`bg-white z-10 rounded-md pb-6 px-3 flex flex-col gap-3 text-center ${
                    item.id === 2 ? 'md:translate-y-7' : 
                    item.id === 3 ? 'md:translate-y-14' : ''
                  }`}>
